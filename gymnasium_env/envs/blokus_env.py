...
from gymnasium_env.envs.auxiliary.precompute_fast_env import load_precomputed, decode
from .blokus_piece import BlokusPieceManager, Vector2
from .blokus_action import BlokusAction
from .blokus_agent_infos import BlokusAgentInfos
from .blokus_theme import BlokusTheme

from gymnasium_env.env_config import PRECOMPUTED_DIR 

class BlokusEnv(gym.Env):
    metadata = {'render_modes': ['human', 'console', 'rgb_array', 'pgf'], 'render_fps': [2, 4, 8]}
    ...
    def _load_neighborhood():
        if BlokusEnv.NEIGHBOR_POS is None:
            BlokusEnv.NEIGHBORHOOD_TO_ENCODED_ACTIONS, BlokusEnv.NEIGHBOR_POS = load_precomputed()
    ...
    def __init__(
        ...
        action_mode: str = "precomputed",
        theme: BlokusTheme = None,
    ):
        ...
        # assert render_fps in self.metadata["metadata["render_fps"]"]
        self.render_mode = render_mode
        self.render_fps = render_fps
        self.metadata["render_fps"] = self.render_fps
        self.theme = theme or BlokusTheme()
        assert action_mode in ["precomputed", "efficient", "inefficient"], f"Invalid action mode: {action_mode}"
        if action_mode == "precomputed":
            BlokusEnv._load_neighborhood()
        # self.indices = np.arange(len(BlokusEnv.NEIGHBOR_POS), dtype=np.uint32)  # [0, 1, 2, ...]
        ...
        self.observation_space = spaces.Dict({
            ...
            "points": spaces.Tuple([
                spaces.Sequence(
                    spaces.Discrete(self.board_size * self.board_size)
                ) for _ in range(self.num_players + 1)
            ]),
            "steps": spaces.Discrete(self.board_size * self.board_size),
        })
    ...
    def reset(
        ...
        *,
        ...
        self.steps = 0
        self.agents_info = BlokusAgentInfos(num_players=self.num_players + 1)
        ...
        if self.num_players == 1:
            ...
            self.agents_info[1].expander_squares = {Vector2(0, 0): Vector2(1, 1)}
            self.agents_info[2].expander_squares = {Vector2(0, self.board_size - 1): Vector2(1, -1)}
            self.agents_info[3].expander_squares = {Vector2(self.board_size - 1, self.board_size - 1): Vector2(-1, -1)}
            self.agents_info[4].expander_squares = {Vector2(self.board_size - 1, 0): Vector2(-1, 1)}
        else:
            raise ValueError("Only 1, 2, or 4 players are supported at the moment.")
    ...
    def _get_obs(self):
        return {
            "num_players": self.num_players,  # Number of players in the game
            ...
            "expander_squares": [None] + [
                np.array(list(self.agents_info[id].expander_squares.keys())) for id in range(1, self.num_players + 1)
            ], # Expander squares for the current player
            "locked_squares": [None] + [
                np.array(list(self.agents_info[id].locked_squares)) for id in range(1, self.num_players + 1)
    ...
    def capture_state(self):
        state = {
            ...
            "points": [self.agents_info[i].points for i in range(1, self.num_players + 1)],
            "steps": self.steps
        }
        ...
        for i in range(1, self.num_players + 1):
            state["agents_info"][i]["available_shapes"] = set(self.agents_info[i].available_shapes)
    ...
    def restore_state(self, state):
        ...
        for i in range(1, self.num_players + 1):
            ...
            self.agents_info[i].available_shapes = set(state["agents_info"][i]["available_shapes"])
            ...
            self.agents_info[i].points = state["points"][i - 1]
    ...
    def place_piece(self, action: BlokusAction, player: int, place=True, check=True) -> bool:
        can_place = True
        if check:
            if not action.piece.shape_id in self.agents_info[player].available_shapes:
    ...
    def update_attributes(self, player, action: BlokusAction):
        ...
        self.agents_info[player].available_shapes.remove(action.piece.shape_id)
    ...
    def possible_actions(self, player: int) -> np.ndarray:
        ...
        if self.action_mode == "precomputed":
            actions = self.possible_actions_precomputed(player)
        elif self.action_mode == "efficient":
            actions = self.possible_actions_efficient(player)
        elif self.action_mode == "inefficient":
            actions = self.possible_actions_inefficient(player)
        else:
    ...
    def possible_actions_efficient(self, player: int) -> np.ndarray:
        ...
        actions = set()
        for shape_id in self.agents_info[player].available_shapes:
    ...
    def possible_actions_precomputed_expander_square(self, expander: Vector2, player: int) -> ActType: # This is the most important (this should be fast)
        ...
        if neighborhood == 0:
            if "1" in self.agents_info[player].available_shapes:
        ...
        if (-2, 0, 16) in actions: # neighborhood & 35 == 35
        ...
        if (0, -2, 13) in actions: # neighborhood & 1792 == 1792
        ...
        for board_row, board_col, piece_id in actions:
            ...
            board_actions.append(
                BlokusAction.tuple_to_id(
                    self.board_size, 
                    int(board_row), 
                    int(board_col), 
                    shape_id, 
                    int(piece_transformation)
                )
            )
    ...
    def step(self, action_id: ActType) -> Tuple[ObsType, float, bool, bool, Dict[str, Any]]:
        ...
        if(not self.place_piece(action=action, player=self.current_player, check=self.testing_mode)):
            print("Invalid move")
    ...
    @contextmanager
    def step_context(self, action_id: ActType):
        state = self.capture_state()
        try:
            result = BlokusEnv.step(self, action_id)
            yield result
        finally:
    ...
    def render(self, path=None):
        ...
        elif self.render_mode == "pgf":
            self._render_pgf(path)

    def _render_pgf(self, path):
        unit = self.theme.render_scale * 4

        fig, ax = plt.subplots(figsize=(self.board_size, self.board_size))
        ax.set_aspect('equal')

        ax.set_facecolor(self.theme.background_color_norm)
        
        for i in range(self.board_size + 1):
            ax.plot([i * unit, i * unit], [0, self.board_size * unit], color=self.theme.grid_color_norm)
            ax.plot([0, self.board_size * unit], [i * unit, i * unit], color=self.theme.grid_color_norm)

        for i in range(self.board_size):
            for j in range(self.board_size):
                if not self.board[i, j]:
                    continue
                player_color = self.theme.cell_color_norm(player_id=self.board[i, j])
                flipped_i = self.board_size - 1 - i
                ax.add_patch(Rectangle((j * unit, flipped_i * unit), unit, unit,
                                    color=player_color))
                
        expander_color = self.theme.expander_color_norm(player_id=self.current_player)
        for i, j in self.agents_info[self.current_player].expander_squares:
            ...
            if self.board[i, j] == 0:
                ...
                num_rings = 10
                max_radius = unit * np.sqrt(2) / 4
                for k in range(num_rings):
                    alpha = 0.5 * (1 - k / (num_rings+1))
                    radius = max_radius * (1 - k / (num_rings+1))
                    ring = Circle((x + unit / 2, y + unit / 2),
                                radius=radius,
        ...
        frame_color = self.theme.cell_color_norm(player_id=self.current_player)
        linewidth = unit * 0.25
        ax.add_patch(Rectangle((-linewidth * 2, -linewidth * 2),
                            self.board_size * unit + linewidth * 4,
                            self.board_size * unit + linewidth * 4,
                            linewidth=linewidth,
        ...
        ax.axis('off')
        plt.savefig(path, format='pgf', bbox_inches='tight')
        plt.close(fig)

    def _render_frame(self):
        cell_size = int(self.theme.render_scale * 4)
        surface_size = self.board_size * cell_size

        if self.window is None and self.render_mode == "human":
            pygame.init()
            pygame.display.init()
            self.window = pygame.display.set_mode((surface_size, surface_size))

        if self.clock is None and self.render_mode == "human":
            self.clock = pygame.time.Clock()

        screen = pygame.Surface((surface_size, surface_size))
        expander_color = self.theme.expander_color(player_id=self.current_player)
        locked_color = self.theme.locked_color(player_id=self.current_player)
        screen.fill(self.theme.background_color)

        # Grid lines
        for i in range(self.board_size + 1):
            pygame.draw.line(screen, (128, 128, 128), (i * cell_size, 0), (i * cell_size, surface_size))
            pygame.draw.line(screen, (128, 128, 128), (0, i * cell_size), (surface_size, i * cell_size))

        # Draw main board cells
        for i in range(self.board_size):
            for j in range(self.board_size):
                if not self.board[i, j]:
                    continue
                player_color = self.theme.cell_color(player_id=self.board[i, j])
                pygame.draw.rect(screen, player_color, pygame.Rect(j * cell_size, i * cell_size, cell_size, cell_size), border_radius=cell_size//4)

        pygame.display.set_caption(f"Blokus - Player {self.current_player}")
        ...
        if self.render_mode == "human":
            for i, j in self.agents_info[self.current_player].locked_squares:
                if 0 <= i < self.board_size and 0 <= j < self.board_size and self.board[i, j] == 0:
                    shift = int(cell_size / 2 - cell_size * np.sqrt(2) / 8)
                    pygame.draw.line(screen, locked_color, (j * cell_size + shift, i * cell_size + shift), ((j + 1) * cell_size - shift, (i + 1) * cell_size - shift), width=3)
                    pygame.draw.line(screen, locked_color, ((j + 1) * cell_size - shift, i * cell_size + shift), (j * cell_size + shift, (i + 1) * cell_size - shift), width=3)

            for i, j in self.agents_info[self.current_player].expander_squares:
                if self.board[i, j] == 0:
                    pygame.draw.rect(screen, expander_color, pygame.Rect(j * cell_size, i * cell_size, cell_size, cell_size), border_radius=cell_size//4)
